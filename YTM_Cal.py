...
st.markdown("""
    <style>
        ...
        /* Consistent input container sizing */
...
with st.container():
    ...
    # Fix grouping to properly align Buy leg
    if freq == 1:
        cf_df["GroupLabel"] = cf_df["Year"].astype(str)
        buy_label = str(settlement_date.year)
    else:
        cf_df["GroupLabel"] = cf_df["Date"].dt.to_period("M").astype(str)
        buy_label = settlement_date.to_period("M").strftime("%Y-%m")

    # Create periodwise cashflows
    periodwise = cf_df.groupby("GroupLabel")["Cashflow"].sum().reset_index()

    # Add the buy leg separately
    periodwise.loc[len(periodwise)] = [buy_label, -price]
    periodwise = periodwise.sort_values("GroupLabel").reset_index(drop=True)
    ...
    fig_bar.add_trace(go.Bar(
        ...
        customdata=periodwise[["HoverLabel"]]
    ))
    ...
    # --- Cumulative Return Chart (same as your code) ---
    fig_cum = go.Figure()
    ...
    fig_cum.add_hline(
        ...
        annotation_text=buy_annotation,
