...
with tabs[1]:
    ...
    if plot_type == "Actual vs Predicted":
        ...
        ax.scatter(y_test, y_pred, alpha=0.6, color=np.random.rand(3,))
        ...
        sns.histplot(residuals, kde=True, bins=30, ax=ax, color=np.random.rand(3,))
        ...
        fig, ax = plt.subplots(figsize=(6, 5))
        sns.scatterplot(x=df['P/E'], y=df['Price'], color=np.random.rand(3,), ax=ax)
        ...
        fig, ax = plt.subplots(figsize=(6, 5))
        sns.scatterplot(x=df['Div Yield %'], y=df['Price'], color=np.random.rand(3,), ax=ax)
...
with tabs[2]:
    ...
    if comparison_option == "Actual vs Predicted":
        with col1:
            fig, ax = plt.subplots()
            ax.scatter(y_test, predictions[base_model], alpha=0.5, color=np.random.rand(3,))
            ax.plot([y.min(), y.max()], [y.min(), y.max()], 'k--')
            ax.set_title(base_model)
            st.pyplot(fig)
        with col2:
            fig, ax = plt.subplots()
            ax.scatter(y_test, predictions[compare_model], alpha=0.5, color=np.random.rand(3,))
            ax.plot([y.min(), y.max()], [y.min(), y.max()], 'k--')
        ...
        with col1:
            fig, ax = plt.subplots()
            sns.histplot(y_test - predictions[base_model], kde=True, bins=30, ax=ax, color=np.random.rand(3,))
            ax.set_title(base_model)
            st.pyplot(fig)
        with col2:
            fig, ax = plt.subplots()
            sns.histplot(y_test - predictions[compare_model], kde=True, bins=30, ax=ax, color=np.random.rand(3,))
            ax.set_title(compare_model)
            st.pyplot(fig)
