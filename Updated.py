...
# --- Dashboard Title ---
st.markdown("""
    <h1 style='text-align: center; font-size: 26px; color: #393E46;'>📊 Financial Model & Behavior Insight Dashboard</h1>
    <p style='text-align: center; color: #6C757D; font-size:13px;'>Analyzing Market Behaviors using Technical Indicators</p>
...
try:
    short_win = 5
    long_win = 10
    df['MA_short'] = df['Price'].rolling(window=short_win).mean()
    df['MA_long'] = df['Price'].rolling(window=long_win).mean()
    df['Volatility_short'] = df['Price'].rolling(window=short_win).std()
    df['Volatility_long'] = df['Price'].rolling(window=long_win).std()
    df.dropna(inplace=True)
    post_feat_rows = len(df)
except:
    post_feat_rows = 0

# --- FEATURE SELECTION ---
if post_feat_rows < 30:
    # Fallback
    features = ['P/E', 'Dividend Yield', 'P/B']
    df = pd.read_csv(github_url, parse_dates=['Date'], index_col='Date')  # Reload raw data
    if "Dividend Yeild" in df.columns:
        df.rename(columns={"Dividend Yeild": "Dividend Yield"}, inplace=True)
...
# --- MODEL INPUTS ---
X = df[features]
y = np.log1p(df['Price'])  # Apply log transform
...
for name, model in model_defs.items():
    ...
    y_pred = np.expm1(y_pred_log)  # Revert log prediction
...
with row1_col1:
    ...
    st.markdown("")
    st.markdown("")
    st.plotly_chart(plot_model_metrics(results_df), use_container_width=True, config={"scrollZoom": True}, height=250)
...
with row1_col2:
    ...
    st.plotly_chart(fig_base, use_container_width=True, config={"scrollZoom": True}, height=250)
...
with row1_col3:
    ...
    st.markdown("")
    st.markdown("")
    st.plotly_chart(fig_compare, use_container_width=True, config={"scrollZoom": True}, height=250)
