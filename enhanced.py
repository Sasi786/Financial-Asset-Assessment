...
with tabs[3]:
    ...
    event_zones = [
        ("2008-09-01", "2009-06-01", "2008 Crash", "red"),
        ("2016-11-01", "2017-02-01", "Demonetisation", "orange"),
        ("2020-03-01", "2021-05-01", "COVID Crash", "purple")
    ]
...
with tabs[4]:
    ...
    market_events = {
        ...
        "COVID-19 Crash": ("2020-02-01", "2020-06-30")
    }
    ...
    # --- 3D Scatter Plot ---
        ...
        def draw_zone_cube(fig, xr, yr, zr, color, name):
            fig.add_trace(go.Mesh3d(
                x=[xr[0], xr[1], xr[1], xr[0], xr[0], xr[1], xr[1], xr[0]],
                y=[yr[0], yr[0], yr[1], yr[1], yr[0], yr[0], yr[1], yr[1]],
                z=[zr[0], zr[0], zr[0], zr[0], zr[1], zr[1], zr[1], zr[1]],
                i=[0, 1, 2, 3, 4, 5, 6, 7, 0, 1, 2, 3],
                j=[1, 2, 3, 0, 5, 6, 7, 4, 4, 5, 6, 7],
                k=[2, 3, 0, 1, 6, 7, 4, 5, 5, 6, 7, 4],
                opacity=0.1, name=name, color=color, showscale=False
            ))
    ...
    # --- Heatmap ---
        ...
        st.subheader("📅 Monthly Market Behavior Heatmap")
        heatmap_df = df.copy()
        heatmap_df['Month'] = pd.to_datetime(heatmap_df['Date']).dt.to_period("M").dt.to_timestamp()
        behavior_map = heatmap_df.groupby(['Month', 'Behavior']).size().reset_index(name='Count')
        pivot_df = behavior_map.pivot(index='Behavior', columns='Month', values='Count').fillna(0)

        fig = go.Figure(data=go.Heatmap(
            z=pivot_df.values, x=pivot_df.columns, y=pivot_df.index,
            colorscale='Inferno', colorbar=dict(title='Count')
        ))

        event_annotations = [
            {'label': '2008 Crisis', 'date': pd.to_datetime('2008-09-15')},
            {'label': 'Demonetization', 'date': pd.to_datetime('2016-11-08')},
            {'label': 'COVID-19 Crash', 'date': pd.to_datetime('2020-03-15')},
        ]
        for event in event_annotations:
            start = event['date'] - pd.DateOffset(months=3)
            end = event['date'] + pd.DateOffset(months=3)
            fig.add_vrect(
                x0=start, x1=end,
        ...
        fig.update_layout(xaxis_title="Month", yaxis_title="Behavior", hovermode="closest")
        st.plotly_chart(fig, use_container_width=True, config={"scrollZoom": True})
